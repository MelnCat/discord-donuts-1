...
const {
	channels: { applicationChannel, kitchenChannel, deliveryChannel, ticketChannel },
	botlists: {
		discordbotsToken,
		discordpwToken,
		discordlistToken,
...
const autoDeliver = async (client, id) => {
	...
	(await client.channels.get(ticketChannel).messages.fetch(finalOrder.ticketMessageID)).delete();
