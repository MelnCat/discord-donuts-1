...
module.exports =
	new DDCommand()
		...
		.setFunction(async (message, args, client) => {
			...
			const claimEmbed = new MessageEmbed().setColor(0x36393E).setDescription(`🎫 **Your order has been claimed by \`${message.author.tag}\`!\nPlease wait while they process your order.**`).setFooter(message.author.tag, message.author.displayAvatarURL()).setTimestamp()
			await client.users.get(order.user).send(claimEmbed).catch(e => { return; });
