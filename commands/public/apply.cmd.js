...
const { everyone, canCook } = require("../../permissions");
	...
	new DDCommand()
		...
		.setFunction(async(message, args, client) => {
			...
			async function getMessage(display) {
				const msg = await message.author.send(display);
				let v = await msg.channel.awaitMessages(m => m.author.id === message.author.id, { max: 1, time: 17000 });
			...
			const embed =
				new DDEmbed(client)
					...
					.setDescription(`Application code: \`${message.author.id.substr(8, 15)}\``)
